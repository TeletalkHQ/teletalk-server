...
function ConsoleBuilder() {
  ...
  const chalkMakerWithBG = (bgColor, data, textColor) => {
    ...
    if (typeof data === "object" && !Array.isArray(data?.values)) {
      this.logs.push(
        chalk[bgColor](
          chalk[data.textColor || textColor || defaultColor](data.text)
        )
      );
    }
    ...
    if (typeof data === "object" && Array.isArray(data?.values)) {
      data.text.forEach((value) => {
        this.logs.push(
          chalk[bgColor](
            chalk[data.textColor || textColor || defaultColor](value)
          )
        );
      });
    }
  ...
  this.bgBlackBright = (
    data = { text: "", textColor: colors.white },
    textColor
